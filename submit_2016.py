...
def create_parametrized_cards(tempDir, tag, params):
    ...
    os.chdir(cwd)
    if not os.path.isdir('../cards_2016/'+tag):
        cmd = 'cp -r cards_2016/{0} ../cards_2016/'.format(tag)
    else:
        cmd = 'cp -r cards_2016/{0}/* ../cards_2016/{0}'.format(tag)
    os.system(cmd)
...
def run_gridpack_generation(tag):
    ...
    if os.path.isdir(tag):
        ...
        os.system('rm -rf '+tag)
        print "Cleaned!"
    cmd = './gridpack_generation.sh {0} cards_2016/{0} local'.format(tag)
    print cmd
    os.system(cmd)
...
def lsf_submit(tag):
    ...
    cwd = os.getcwd()
    os.chdir(cwd+'/../')
    #cmd = './submit_gridpack_generation.sh 15000 15000 2nw {0} cards_2016/{0} 8nh'.format(tag)
    cmd = './submit_gridpack_generation.sh 15000 15000 8nh {0} cards_2016/{0} 8nh'.format(tag)
    os.system(cmd)
...
def condor_submit(tag):
    ...
    cwd = os.getcwd()
    os.chdir(cwd+'/../')
    cmd = 'PYTHONPATH=$PYTHONPATH:/usr/lib64/python2.7/site-packages; ./submit_condor_gridpack_generation.sh {0} cards_2016/{0}'.format(tag)
    os.system(cmd)
    cmd = 'mv %s_slc6_amd64_gcc481_CMSSW_7_1_30_tarball.tar.xz %s.tar.xz' % (tag, tag)
    os.chdir(cwd)
